            ...
            if ($rolloverType) {
                ...
                if ((Test-Path -Path $Configuration.Path) -and ((Get-Date)-(Get-ChildItem -Path $Configuration.Path | Select -ExpandProperty LastWriteTime)).Days -gt 0)
                {
                    try {
                        ...
                        Write-Verbose "[Logger/CMTrace] Rollover file ($($Configuration.Path)) removed"
                    } catch {
                        Write-Verbose "[Logger/CMTrace] Rollover file ($($Configuration.Path)) failed to be removed : $_"
                    }
                }
            }
